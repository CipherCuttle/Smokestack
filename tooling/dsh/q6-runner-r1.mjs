...
const invalidV0Json = path.join(evidenceDir, 'Q6_COMPARATIVE_UTILITY_INVALID_V0.json');
const invalidV0Md = path.join(evidenceDir, 'Q6_COMPARATIVE_UTILITY_INVALID_V0.md');
...
function runAsync(cmd, args, opts = {}) {
  ...
  return new Promise((resolve) => {
    ...
    const finish = (exit, signal, error = null) => {
      ...
      console.log(`Q6_PHASE_DONE ${label} exit=${out.exit} duration_ms=${out.duration_ms}`);
      resolve(out);
    };
...
function preserveInvalidV0() {
  if (!fs.existsSync(evidenceJson)) return;
  try {
    const old = readJson(evidenceJson);
    const s = old?.summary ?? {};
    const A = s.A_CODEX_ALONE ?? {};
    const B = s.B_CODEX_PROMPT_SELF_REVIEW ?? {};
    const C = s.C_DSH_DEEPSEEK_CODEX_CLAUDE ?? {};
    const suspicious =
      Number(A.diff_lines ?? -1) === 0 &&
      Number(B.diff_lines ?? -1) === 0 &&
      Number(C.diff_lines ?? -1) === 0 &&
      Number(C.parent_requests ?? -1) === 0 &&
      Number(C.wall_ms ?? 0) >= 800_000;
    if (suspicious && !fs.existsSync(invalidV0Json)) {
      fs.copyFileSync(evidenceJson, invalidV0Json);
      const md = `# Q6 Comparative Utility V0 — Invalid Evaluator Lineage\n\nStatus: \`BLOCKED_BY_EVALUATOR_IMPLEMENTATION\`\n\nThe first Q6 run is preserved but is not admissible evidence about DSH utility.\n\nRoot causes:\n\n1. Direct Codex arms placed the global \`--ask-for-approval\` option after the \`exec\` subcommand, so current Codex rejected the invocation before model execution.\n2. The DSH arm hosted its OpenRouter proxy in the evaluator process but launched DSH with \`spawnSync\`, blocking the Node event loop and deadlocking the proxy until each timeout.\n\nObserved symptom set: zero diffs in all arms, zero parent requests/tokens/cost in C, and ~900 seconds C wall time.\n\nThe resulting \`DSH_BUILD_CONTROL_REJECTED\` decision is invalid and MUST NOT be interpreted as treatment evidence.\n`;
      writeFile(invalidV0Md, md);
      console.log('Q6_R1: preserved invalid Q6 V0 evidence lineage.');
    }
  } catch (err) {
    fail(`could not inspect/preserve prior Q6 evidence: ${err.message}`);
  }
}
...
function gitInit(dir, files) {
  ...
  let r = runSync('git', ['init', '-q'], { cwd: dir });
  if (r.exit) fail(`git init failed: ${r.stderr}`);
  runSync('git', ['config', 'user.email', 'smokestack-q6@example.invalid'], { cwd: dir });
  runSync('git', ['config', 'user.name', 'Smokestack Q6'], { cwd: dir });
  r = runSync('git', ['add', '.'], { cwd: dir });
  if (r.exit) fail(`git add failed: ${r.stderr}`);
  r = runSync('git', ['commit', '-qm', 'Q6 baseline'], { cwd: dir });
  if (r.exit) fail(`git commit failed: ${r.stderr}`);
}

function changedPaths(dir) {
  const r = runSync('git', ['status', '--porcelain'], { cwd: dir });
  if (r.exit) return ['<git-error>'];
  return r.stdout.trim().split('\n').filter(Boolean).map((line) => line.slice(3));
}

function diffSize(dir) {
  const r = runSync('git', ['diff', '--numstat'], { cwd: dir });
...
const tasks = [
  {
    ...
    hidden(dir) {
      ...
      const r = runSync(process.execPath, ['-e', code], { cwd: dir });
...
function visibleTest(dir) {
  const r = runSync(process.execPath, ['--test', 'visible.test.js'], { cwd: dir, timeoutMs: 60_000 });
...
function evaluate(dir, task) {
  ...
  return {
    visible_exit: visible.exit,
    hidden_exit: hidden.exit,
    severe_failures: hidden.severe_failures,
    scope,
    diff: diffSize(dir),
  };
}
...
function directCodex(dir, prompt, label) {
...
async function startProxy(port = 0, cap = 8) {
  const state = {
    max_requests: cap,
    allowed_requests: 0,
    blocked_requests: 0,
  ...
  const server = http.createServer((req, res) => {
    if (req.method === 'POST' && req.url?.startsWith('/api/v1/')) {
      ...
      console.log(`Q6_PARENT_REQUEST ${state.allowed_requests}/${cap}`);
    }
    ...
    const upstream = https.request({
      ...
      u.on('end', () => {
        ...
        let lastUsage = null;
        for (const obj of candidates) if (obj?.usage) lastUsage = obj.usage;
        if (lastUsage) {
...
async function treatmentA(task) {
  ...
  const started = Date.now();
  const impl = await directCodex(dir, promptForTask(task), `${task.id}/A/CODEX`);
  const final = evaluate(dir, task);
  return {
    treatment: 'A_CODEX_ALONE',
    task: task.id,
    wall_ms: Date.now() - started,
    child_calls: 1,
    repair_calls: 0,
    implementation_exit: impl.exit,
    implementation_stderr_tail: impl.stderr.slice(-2000),
    final,
  };
}
...
async function treatmentB(task) {
  ...
  gitInit(dir, task.files);
  const started = Date.now();
  const impl = await directCodex(dir, promptForTask(task), `${task.id}/B/IMPLEMENT`);
  const review = await directCodex(dir, codexSelfReviewPrompt(task), `${task.id}/B/SELF_REVIEW`);
  ...
  return {
    ...
    wall_ms: Date.now() - started,
    child_calls: 2,
    repair_calls: gate === 'CRITICAL_HIGH_FOUND' ? 1 : 0,
    implementation_exit: impl.exit,
    review_exit: review.exit,
    review_gate: gate,
    implementation_stderr_tail: impl.stderr.slice(-2000),
    review_stderr_tail: review.stderr.slice(-2000),
    final,
  };
}

async function treatmentC(task) {
  ...
  let reviewCalls = 0;
  try {
    let patches = dshPatchFiles(dir, proxy.port, 'implement');
    const implPrompt = `Q6 R1 IMPLEMENT. Call subagent_codex_implementer exactly once and no other tool. Give Codex this standalone task:\n\n${promptForTask(task)}\n\nAfter successful completion reply exactly DSH_Q6_IMPLEMENT_OK.`;
    const impl = await dshRun(dir, patches, implPrompt, `${task.id}/C/IMPLEMENT`);
    childCalls++;
    ...
    if (review.exit !== 0 || gate === 'AMBIGUOUS') {
      return {
        ...
        rereview_exit: null,
        rereview_gate: null,
        terminal: 'AMBIGUOUS_REVIEW_FAILURE',
        review_stderr_tail: review.stderr.slice(-2000),
        parent: { ...proxy.state },
        final: evaluate(dir, task),
      };
    ...
    if (gate === 'CRITICAL_HIGH_FOUND') {
      ...
      if (repair.exit !== 0 || postRepairVisible.exit !== 0 || !postRepairScope.ok) {
        return {
          ...
          review_exit: review.exit,
          review_gate: gate,
          repair_exit: repair.exit,
          rereview_exit: null,
          rereview_gate: null,
          terminal: 'REPAIR_OR_HOST_RETEST_FAILURE',
          repair_stderr_tail: repair.stderr.slice(-2000),
          parent: { ...proxy.state },
          final: evaluate(dir, task),
        };
      }
...
function evaluatorValidity(results, summary) {
  ...
  for (const r of results) {
    if (r.implementation_exit !== 0) reasons.push(`${r.task}/${r.treatment}: implementation executor exit=${r.implementation_exit}`);
    if (r.treatment === 'B_CODEX_PROMPT_SELF_REVIEW' && r.review_exit !== 0) reasons.push(`${r.task}/B: self-review executor exit=${r.review_exit}`);
    if (r.treatment === 'C_DSH_DEEPSEEK_CODEX_CLAUDE') {
      if ((r.parent?.allowed_requests ?? 0) < 1) reasons.push(`${r.task}/C: zero parent requests observed`);
      if (r.review_exit == null) reasons.push(`${r.task}/C: review phase not reached`);
    }
  }
  if (summary.A_CODEX_ALONE.wall_ms < 1000) reasons.push('A aggregate wall time implausibly short for three model-backed Codex tasks');
  if (summary.C_DSH_DEEPSEEK_CODEX_CLAUDE.parent_requests < 3) reasons.push('C parent request count below one per task');
...
function decide(summary, validity) {
  ...
  if (!validity.valid) {
    return {
      ...
      reason: 'Q6 evaluator/executor validity gate failed',
      evaluator_validity: validity,
      safety,
      non_inferior: nonInferior,
      governance,
      governance_score: governanceScore,
      overhead_ratio_vs_b: overheadRatioVsB,
      cost_accounting_observed: costKnown,
    };
  }
  ...
  if (safety && nonInferior && governanceScore >= 5 && costKnown && overheadRatioVsB != null && overheadRatioVsB <= 3.0) {
    ...
    reason = C.hidden_pass > Math.max(A.hidden_pass, B.hidden_pass)
      ...
      : 'non-inferior quality plus stronger mechanical governance at bounded orchestration cost';
...
function processQuiescence() {
  const r = runSync('ps', ['-eo', 'pid=,args='], { timeoutMs: 30_000 });
...
function preflight() {
  ...
  const cv = runSync(codexBin, ['--version'], { env: { CODEX_HOME: codexHome }, timeoutMs: 30_000 });
  if (cv.exit) fail(`Codex version probe failed: ${cv.stderr}`);
  const login = runSync(codexBin, ['login', 'status'], { env: { CODEX_HOME: codexHome }, timeoutMs: 30_000 });
  if (login.exit) fail(`Codex auth probe failed: ${login.stderr}`);
  const argvProbe = runSync(codexBin, ['--ask-for-approval', 'never', '--sandbox', 'workspace-write', 'exec', '--help'], { env: { CODEX_HOME: codexHome }, timeoutMs: 30_000 });
  if (argvProbe.exit) fail(`Codex exec argv probe failed: ${argvProbe.stderr}`);
  const dsh = runSync('smokestack-dsh', ['--profile', 'headless', '--dump-config'], { timeoutMs: 60_000 });
  if (dsh.exit || dsh.stderr.trim()) fail(`DSH config probe failed: exit=${dsh.exit} stderr=${dsh.stderr}`);
  return {
    node,
    codex: cv.stdout.trim() || cv.stderr.trim(),
    codex_login: login.stdout.trim() || login.stderr.trim(),
    codex_exec_argv_probe: 'PASS',
  };
}
...
async function main() {
  ...
  for (const task of tasks) {
    ...
    ordinal++;
    console.log(`\n=== Q6 R1 [${ordinal}/${total}] ${task.id} / A ===`);
    results.push(await treatmentA(task));

    ordinal++;
    console.log(`=== Q6 R1 [${ordinal}/${total}] ${task.id} / B ===`);
    results.push(await treatmentB(task));

    ordinal++;
    console.log(`=== Q6 R1 [${ordinal}/${total}] ${task.id} / C ===`);
    results.push(await treatmentC(task));
  }
  ...
  const receipt = {
    qualification: 'DSH_Q6_COMPARATIVE_UTILITY_R1',
    predecessor_invalid_lineage: 'Q6_V0_BLOCKED_BY_EVALUATOR_IMPLEMENTATION',
    ...
    design: {
      ...
      child_call_measurement: 'controller-authorized delegation phases; not a claim of a preventive within-turn subagent invocation counter',
      decision_rule: 'Evaluator must first pass executor/transport validity. Then C must be perfect on visible/hidden host tests, have zero unauthorized writes/severe escapes, be non-inferior to A/B, demonstrate the listed governance properties, expose parent usage/cost accounting, and stay <=3x B wall-clock.',
    },
    evaluator_validity: validity,
    summary,
    decision,
